...
def matrix_cofactor(array):
	...
	#array=None
	#TODO
	get_inverse=np.linalg.inv(array)
	det=np.linalg.det(array)
	get_inverse=det*get_inverse
	array=np.transpose(get_inverse)
	return array
...
def f1(X1,coef1,X2,coef2,seed1,seed2,seed3,shape1,shape2):
	...
	mul1= np.matmul(W1,np.linalg.matrix_power(X1, coef1))
	mul2= np.matmul(W2,np.linalg.matrix_power(X2, coef2))
